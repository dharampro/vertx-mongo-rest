...
public class UserHandler {
  ...
  public void userProduct(RoutingContext rc) {
    String taskId = rc.request().getParam("taskId");
    rc.vertx().eventBus().request("user.product.fetch.ops.bus", taskId).onComplete(result -> {
      if (result.succeeded()) {
        UserFetchResponse response = new UserFetchResponse(JsonObject.mapFrom(result.result().body()));
        rc.response()
          .setStatusCode(HttpResponseStatus.OK.code())
  ...
  public void userProductSubscribe(RoutingContext rc) {
    rc.request().bodyHandler(handler -> {
      rc.vertx().eventBus().request("user.product.subscribe.ops.bus", handler.toJsonObject()).onComplete(result -> {
        if (result.succeeded()) {
          rc.response()
            .setStatusCode(HttpResponseStatus.OK.code())
            .putHeader("Content-Type", "application/json")
